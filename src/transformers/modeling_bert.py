...
class BertEmbeddingsIncr(BertEmbeddings):
    def __init__(self, config):
...
class BertSelfAttentionIncr(BertSelfAttention):
...
class BertAttentionIncr(BertAttention):
    def __init__(self, config):
        super().__init__(config)
...
class BertLayerIncr(BertLayer):
    def __init__(self, config):
        super().__init__(config)
        self.attention = BertAttentionIncr(config)
        self.intermediate = BertIntermediate(config)
        self.output = BertOutput(config)
...
class BertEncoderIncr(BertEncoder):
    def __init__(self, config):
        super().__init__(config)
        self.layer = nn.ModuleList([BertLayerIncr(config) for _ in range(config.num_hidden_layers)])
...
class BertLMPredictionHeadIncr(BertPreTrainingHeads):
...
class BertPreTrainingHeadsIncr(BertPreTrainingHeads):
    def __init__(self, config, bert_model_embedding_weights, num_labels=2):
        super().__init__(config)
        self.predictions = BertLMPredictionHeadIncr(config, bert_model_embedding_weights)
...
class BertModelIncr(BertModel):
    def __init__(self, config):
        super().__init__(config)
        self.embeddings = BertEmbeddingsIncr(config)
        self.encoder = BertEncoderIncr(config)
...
class BertForImg2Txt(BertPreTrainedModel):
    ...
    def __init__(self, config, mask_word_id=0, num_labels=2,
                 ...
                 ngram_size=3, min_len=0, len_vis_input=49):
        super().__init__(config)
        self.bert = BertModelIncr(config)
        self.cls = BertPreTrainingHeadsIncr(config, self.bert.embeddings.word_embeddings.weight, num_labels)
    ...
    def forward(self, vis_feats, vis_pe, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None,
                sample_mode='greedy'):
        ...
        while next_pos < output_length:
            ...
            if sample_mode == 'greedy':
                ...
                max_ids = torch.multinomial(prediction_probs, num_samples=1, replacement=True)
                max_probs = torch.gather(F.log_softmax(prediction_scores, dim=-1), 1, max_ids)  # this should be logprobs
            else:
                raise NotImplementedError
