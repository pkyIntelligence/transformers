...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, vis_feats, vis_pe, input_ids, token_type_ids=None, position_ids=None, inputs_embeds=None,
                vis_input=True, len_vis_input=49):
        ...
        if vis_input:
            inputs_embeds = torch.cat((inputs_embeds[:, :1], vis_feats, inputs_embeds[:, len_vis_input+1:]), dim=1)
            assert len_vis_input == 100, 'only support region attn!'
            position_embeddings = torch.cat((position_embeddings[:, :1], vis_pe,
                                             position_embeddings[:, len_vis_input+1]), dim=1)
